...
def cmd_train(a):
    ...
    cls.train(
        ...
        Path(a.annotation),
        out,
        init_dir=Path(a.init) if a.init else None,
        **hparams,
    )
...
def _calibrate(md: Path, genome: Path, annotation: Path, chroms: list[str] | None):
    ...
    for t, v in report["params"].items():
        print(
            f"calibrate {t}: a={v['a']:.3f} b={v['b']:.3f} n+={v['n_pos']} "
            f"ECE {v['ece_before']:.4f} -> {v['ece_after']:.4f}",
            file=sys.stderr,
        )
...
def main(argv=None):
    p = argparse.ArgumentParser(
        prog="sitescore", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter
    )
    ...
    t.add_argument(
        "--no-calibrate", action="store_true", help="skip the Platt fit on the validation sequences"
    )
    ...
    c.add_argument(
        "--chroms", help="comma list of sequences to fit on (default: train_info.json val_chroms)"
    )
    ...
    s.add_argument(
        "--raw", action="store_true", help="emit raw probabilities even if calibration.json exists"
    )
